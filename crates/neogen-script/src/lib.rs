...
//! system Lua) and will host the script sandbox (2.2), the per-tick
//! instruction budget (2.3), the `move/scan/act/print` API (2.4) and the
//! multi-script scheduler (2.6). Like `neogen-core`, this crate never
//! depends on Godot or GDExtension bindings: the simulation stays
//! headless-testable, the bridge consumes it from `neogen-gdext`.
//!
...
impl Runtime {
    /// Create a runtime with Lua 5.4 and the safe standard library subset
    /// loaded (the full sandbox policy arrives in backlog 2.2).
    ///
    /// Construction cannot fail yet; the `Result` is part of the contract
    /// because the sandbox setup (2.2) will be fallible.
